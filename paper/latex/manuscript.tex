...
The complete test-input batch supplies monitoring scores with true labels hidden. Label-cost savings exclude training, segmentation and participant setup. The stopping guarantee uses monitoring-window weighting, whereas reported coverage and set size use participant summaries. The stopping statistic averages set size over windows in that batch. Empirical coverage and set size are also summarized with equal weight per participant. These two aggregations are retained explicitly in the result tables.
...
\botrule
\end{tabular}
\par\vspace{4pt}\raggedright\footnotesize LAC at $\delta=0.05$ and $\tau=0.5$. CS$(a,b)$ uses a Beta$(a,b)$ mixture; values use participant weighting. This diagnostic follows confirmation.\par
\end{minipage}
\par\vspace{14pt}
\begin{minipage}{0.9\textwidth}
\centering\small\setlength{\tabcolsep}{4pt}
...
\botrule
\end{tabular}
\par\vspace{4pt}\raggedright\footnotesize Five repetitions on the stated CPU at $\delta=0.05$. Build time is per pool/checkpoint design; query time uses cached boundaries.\par
\end{minipage}
\end{figure*}
...
\botrule
\end{tabular}
\par\vspace{4pt}\raggedright\footnotesize Saved (\%) is $100(1-T/N)$ relative to the full reference pool. CS$(a,b)$ denotes the Beta prior; failure rates count query campaigns.\par
\end{minipage}
