...
\affiliation[aff1]{organization={Xinjiang University},city={Urumqi},country={China}}
\cortext[cor1]{Corresponding author}
\fntext[orf]{ORCID: \url{https://orcid.org/0009-0008-3242-7961}}
\fntext[ory]{ORCID: \url{https://orcid.org/0009-0009-9295-5149}}
\begin{abstract}
Set-valued prediction supports decisions when several outcomes remain plausible, but acquiring the labels required for calibration can be costly. We study sequential label acquisition relative to a fixed finite reference predictor. Partial labels induce an interval for the reference quantile and a corresponding enclosure of nested prediction sets. We develop an acquisition procedure that refines this enclosure using finite-population rank bounds and stops when its mean set-size difference on an unlabeled monitoring batch reaches a prescribed tolerance. Joint rank calibration (JRC) uses a rank-state recursion to calibrate both boundaries over the declared query checkpoints. Under uniform sampling without replacement, the resulting confidence event preserves the reference sets for every input and bounds their average enlargement on the monitoring batch. Boundary tables are computed before querying and reused across pools with the same design. The evaluation comprises 354 classifier cases on seven public wearable datasets and 45 additional HHAR cases. At confidence error 0.05 and tolerance 0.5, JRC saves 23.67\% of reference labels on USC-HAD, compared with 17.91\% for Bonferroni hypergeometric bounds and 11.74\% for a uniform-prior confidence sequence. The strongest reported prior variant reaches 15.33\%. On HHAR, savings are 34.03\% for JRC and 25.13\% for the fixed Beta$(9,1)$ comparator. These results characterize the relation between acquisition effort, rank uncertainty and the precision of set-valued decisions.
\end{abstract}
\begin{keyword}
...
Set-valued prediction represents uncertainty by retaining several admissible outcomes for a decision. Its usefulness depends on both inferential reliability and the precision of the returned set. Conformal prediction provides a framework for constructing prediction sets with statistical validity \cite{vovk,angelopoulos}, and its connections with consonant plausibility measures link this framework to broader approaches to uncertain reasoning \cite{cella2022}. Acquiring the labels needed to construct a prediction set can itself be costly. This raises a decision problem: how much calibration information is needed before further labeling has little effect on the prediction sets of interest?

We study this problem relative to a fixed calibration reference. A trained classifier, a finite pool of inputs and a score function define the prediction sets that would be obtained after labeling the entire pool. During acquisition, the labels are revealed sequentially, so the reference quantile is only partially known. Bounds on that quantile induce lower and upper prediction sets. Their inclusion relation describes what is already determined and what remains unresolved about the reference decision.

Our acquisition procedure refines this set enclosure until its remaining imprecision reaches a specified tolerance. Before querying, a finite-population rank recursion calibrates the quantile boundaries across the declared checkpoints. At each checkpoint, candidate-label scores on an unlabeled monitoring batch translate the threshold interval into a mean difference in set size. Querying stops when this difference is sufficiently small, and the procedure returns the upper-endpoint sets. The resulting confidence event simultaneously preserves the reference sets for every input and controls their average enlargement on the monitoring batch.

The method connects two quantities with different roles: a probability budget for errors in reference preservation and a cardinality budget for the additional candidate labels. The cardinality criterion expresses precision in the space of decisions. It is invariant to strictly increasing score transformations and can be evaluated without monitoring labels. We implement the procedure using joint rank calibration (JRC), with boundary tables that depend on the finite query design and can be reused across reference pools.

The contributions are:
\begin{enumerate}
\item A sequential reference-preservation formulation that expresses acquisition uncertainty through nested prediction sets and a mean set-size tolerance.
\item A finite-checkpoint rank-calibration procedure and an observable stopping rule, with a joint containment and inflation guarantee under the specified query design.
\item An empirical characterization on seven public wearable datasets with 354 classifier cases and an additional HHAR evaluation with 45 cases, relating label cost to boundary construction, reference-set quality and stopping tolerance.
\end{enumerate}
Wearable activity recognition provides the application setting: repeated measurements, user heterogeneity and imperfect probability calibration make the precision of prediction sets consequential \cite{guo,ovadia}. The reported USC-HAD comparison yields 23.67\% reference-label savings for JRC, 17.91\% for Bonferroni hypergeometric bounds and 11.74\% for the uniform-prior confidence sequence. The prior sensitivity and additional HHAR study characterize how these results depend on the comparator and reference design.
...
The eight public datasets are available from the cited providers \cite{har,dsads,mhealth,wisdm,pamap,realdisp,uschad,hhar}. Supplementary material 1 contains the frozen predictions, result tables, protocols and verification records for the reported experiments.

\section*{Code availability}
Code and reproduction instructions are available at \url{https://github.com/Cyrano666/jrc-reference-emulation}. The accompanying archive preserves the reported study version.
